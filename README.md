...
在 VSCode 中按 `F5` 启动调试，会打开一个扩展开发宿主窗口。
...
# 或指定输出文件名
npx vsce package -o wechat-pub-0.1.0.vsix
```

生成的 `.vsix` 文件可以：
- 在 VSCode 中手动安装：`Extensions: Install from VSIX...`
- 发布到 VSCode Marketplace
- 分享给其他人直接安装

### 发布到 Marketplace

1. 在 [Azure DevOps](https://dev.azure.com) 创建 Personal Access Token（需要 `Marketplace > Manage` 权限）
2. 使用 `vsce` 登录并发布：
